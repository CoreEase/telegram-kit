...
export function createMockWebApp(options: DevModeOptions = {}): TgWebApp {
  ...
  const mock: MockWebApp = {
    ...
    close() { console.warn('[telegram-kit dev] WebApp.close() called'); },
    sendData(data) { console.info('[telegram-kit dev] sendData:', data); },
    switchInlineQuery(query, types) { console.info('[telegram-kit dev] switchInlineQuery:', query, types); },
    openLink(url) { window.open(url, '_blank'); },
    openTelegramLink(url) { window.open(url, '_blank'); },
    openInvoice(_url, cb) { cb?.('cancelled'); },
    shareToStory(url, params) { console.info('[telegram-kit dev] shareToStory:', url, params); },
    shareMessage(msgId, cb) { console.info('[telegram-kit dev] shareMessage:', msgId); cb?.(false); },
    setEmojiStatus(_id, _p, cb) { cb?.(false); },
    requestEmojiStatusAccess(cb) { cb?.(false); },
    downloadFile(params, cb) { console.info('[telegram-kit dev] downloadFile:', params); cb?.(false); },
    hideKeyboard() {},
    requestChat(params, cb) { console.info('[telegram-kit dev] requestChat:', params); cb?.(false); },

    showPopup(params, cb) {
      const id = params.buttons?.[0]?.id ?? null;
      alert(`[telegram-kit dev popup] ${params.title ? params.title + '\n' : ''}${params.message}`);
      cb?.(id);
    },
    showAlert(message, cb) { alert(`[telegram-kit dev] ${message}`); cb?.(); },
    showConfirm(message, cb) { cb?.(confirm(`[telegram-kit dev] ${message}`)); },
    showScanQrPopup(_params, cb) {
      const text = prompt('[telegram-kit dev] Enter QR text:') ?? '';
    ...
    invokeCustomMethod(method, params, cb) {
      console.info('[telegram-kit dev] invokeCustomMethod:', method, params);
      cb?.(null, { ok: true });
    },
    ...
    LocationManager: (() => {
      ...
      const loc: LocationManager = {
        ...
        openSettings() { console.info('[telegram-kit dev] LocationManager.openSettings()'); return loc; },
      };
      return loc;
    })(),
...
export function installDevMode(options: InstallDevModeOptions = {}): boolean {
  ...
  console.info(
    '%c[telegram-kit] Dev mode active',
...
function _attachDevIndicator() {
  ...
  if (document.getElementById('telegram-kit-dev-indicator')) return;
  ...
  el.id = 'telegram-kit-dev-indicator';
  el.textContent = 'telegram-kit dev';
  el.style.cssText = [
    'position:fixed',
    'bottom:8px',
    'right:8px',
