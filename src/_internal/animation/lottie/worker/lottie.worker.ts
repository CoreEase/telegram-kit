...
import { DEFAULT_RENDER_QUALITY, type RenderQuality } from "../core/types";
import { loadWasmModule, type LottieWasmExports } from "../core/wasm";
import type {
	MainToWorkerMessage,
	WorkerInitConfig,
	WorkerToMainMessage,
...
function getWasm(wasmUrl: string | undefined): Promise<LottieWasmExports> {
	if (!wasmUrl)
		return Promise.reject(
			new Error("lottie: worker received no wasmUrl to load"),
		);
...
class WorkerAnimation {
	...
	private readonly wasm: LottieWasmExports;
	...
	private instance: LottieInstance;
	...
	constructor(
		...
		if (!ctx2d)
			throw new Error(
				"lottie: failed to acquire a 2d context from the OffscreenCanvas",
			);
		this.ctx2d = ctx2d;
	...
	recolor(
		...
		const next = LottieInstance.create(this.wasm, this.animationBytes, {
			fitzModifier,
		...
		if (!next) {
			this.post({
				...
				error: {
					reason: "parse",
					message: "lottie rejected the recolor options",
				},
			});
			return;
		}
...
async function handleInit(id: string, config: WorkerInitConfig): Promise<void> {
	...
	const instance = LottieInstance.create(wasm, bytes, {
	...
	if (!instance) {
		ctx.postMessage({
			...
			error: {
				reason: "parse",
				message: "lottie rejected the animation data",
			},
		} satisfies WorkerToMainMessage);
		return;
	}
