...
export function encodeStartParam(value: string): string {
  ...
  if (encoded.length > 64) {
    throw new Error(
      `[telegram-kit] encodeStartParam: the encoded value is ${encoded.length} characters, ` +
...
function resolveStartParam(options: StartParamInput): string | undefined {
  if (options.startParam !== undefined && options.data !== undefined) {
    throw new Error('[telegram-kit] Provide either `startParam` or `data`, not both.');
  }
  ...
  if (options.startParam !== undefined) {
    if (!isValidStartParam(options.startParam)) {
      throw new Error(
        `[telegram-kit] "${options.startParam}" is not a valid Telegram start parameter ` +
